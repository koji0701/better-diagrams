        ...
        tools: [
            {
                ...
                description:
                    "Convert Mermaid diagram syntax to clean ASCII art. Supports flowcharts, sequence diagrams, and more.",
                inputSchema: {
            ...
            },
            {
                name: "create_diagram",
                description:
                    "Create an ASCII box diagram from structured JSON. Best for architecture diagrams with boxes and connections.",
                inputSchema: {
                    type: "object" as const,
                    properties: {
                        boxes: {
            ...
            },
            {
                name: "refine_ascii",
                description:
                    "Fix a broken ASCII diagram. Repairs alignment issues, closes unclosed boxes, straightens arrows. Use this when a cheaper model generates a rough diagram that needs cleanup.",
                inputSchema: {
                    type: "object" as const,
                    properties: {
                        ascii: {
            ...
            },
            {
                name: "edit_diagram",
                description:
                    "Edit an existing ASCII diagram by finding a box and replacing its content. The box will auto-resize to fit the new text. Use this to update labels without breaking the diagram structure.",
                inputSchema: {
                    type: "object" as const,
                    properties: {
                        ascii: {
    ...
    try {
        switch (name) {
            case "mermaid_to_ascii": {
                const result = await mermaidToAscii(args?.mermaid as string);
            ...
            case "refine_ascii": {
                const result = refineAscii(args?.ascii as string);
                return { content: [{ type: "text", text: result }] };
            }
            case "edit_diagram": {
                const result = editDiagram(
                    args?.ascii as string,
                    args?.operations as Array<{ find: string; replace: string }>
                );
                return { content: [{ type: "text", text: result }] };
            }
            default:
