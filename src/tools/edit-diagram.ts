 ...
 */
function findAllBoxes(lines: string[]): BoxMatch[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        for (let j = 0; j < line.length; j++) {
            ...
            if (char === "┌" || char === "+" || char === "╭") {
                ...
                if (endCol < 0 || endCol >= line.length) continue;
 ...
 */
function createBox(content: string, minWidth = 0): string[] {
...
export function editDiagram(
    ...
    for (const op of operations) {
        ...
        const targetBox = boxes.find((box) =>
            box.content.toLowerCase().includes(op.find.toLowerCase())
        );
